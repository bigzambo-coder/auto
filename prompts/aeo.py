...
네이버 SEO 텍스트 분량 원칙:
- 이미지 <div> placeholder 태그는 텍스트가 아님 — 글자 수 계산에서 완전 제외
- 이미지가 1장이든 7장이든 순수 텍스트 분량은 항상 동일해야 함
- AEO SEO 최적 텍스트: 순수 텍스트 기준 4500~6500자 목표 (FAQ 포함 시 더 길게)
- 각 <h2> 섹션: 최소 400자 이상의 텍스트 (이미지 제외)
...
두괄식 요약·테이블·FAQ 사이나 중요 단락 뒤에 균등하게 삽입하세요.
...
예시:
<div class="img-placeholder" data-prompt="infographic showing comparison chart with colorful icons and statistics" data-kr="각 항목을 한눈에 비교할 수 있는 컬러풀한 인포그래픽 비교표" data-idx="1"></div>
...
- [본문]:
  ...
  * [텍스트 분량] 순수 텍스트 4500~6500자 목표 (이미지 <div> 태그는 0자로 계산, 이미지 장수와 완전히 무관)
  * <h2> 소제목 4~6개, 각 섹션 400자 이상 풍부한 텍스트 → 이미지 삽입 후에도 텍스트는 계속 이어짐
